...
create user account:
    sudo useradd recorder
    sudo passwd recorder
    sudo usermod -aG sudo recorder

create user account plus own home directories etc:
    sudo adduser recorder

last -- shows login history (check for crackers) -- data in /use/adm or similar (a good cracker would remove their logins from this though)


useradd -m charles   #m create homedir
userdel charles

change user name (same internal uID number)
usermod -l newname oldname

groupadd myname

swapping keys
~/.xmodmap   #need to remove the 'lock' on each key, then update it, then re-lock it

autologin (xubuntu only?)
...
to add jobs:
crontab -e     
nb: MUST end with a return character or will not work!

eg.
11 45 * * * touch full/path/to/foo <CR>

this runs the touch cmd at 11:45 every day.
nothing further to do -- auto updates everything, and cron is (usually) always running on linux machines.


If cmd needs to be done as root, then must add as
sudo crontab -e
(this adds to root's cronjobs rather than a users)
(eg useful for rebooting a machine every day)
...
- Ethernet is a low level protcol in which each device on a physical network as a MAC address and can send and receive "frames" (not "packets") of data to another device via its MAC address. MAC addresses are (almost) hardwired into physical network cards. Ethernet is a bus so any frames put on the bus are visible to everyone on the bus.
...
- TCP is higher level still, and is a protocol which control how IP packets are passed around the (inter)network.

 (ICMP is another for sending error messages such as "host unreachable")
...
As well as routing approrpiate traffic between the two cards, it can be useful to use this machine to host other network services for the local network too, as below.
...
See tools: nslookup, whois, host

\section{Static IP}

set hostname: /etc/hosts    #local DNS lookup 
        /etc/hostname   #or use hostname command 
 
remove network-manager (ubuntu,apt-get) NetworkManager(Fedora,yum) package 
    this is an high level layer of aumated conenctions, eg auto switch from wifi to ethernet, when cable plugged in 
    it gets in the way of manual edits. 
 
if card not shown in ifconfig, it may be DOWN: 
...
should be able to manual assign IPs and ping between them here.


\section{Routing to the internet}
...
NB for applications such as outdoor robots where network contact with
base may be lost occasionally, it may be best to configure everything
with static IPs so that no-one is reliant on any central DHCP server.
...
``named''. DNS runs as a service on port 53 of the gateway. (Other
...
If devies identical, they can be distinguished by the physical USB port number which they are connected to:
http://askubuntu.com/questions/49910/how-to-distinguish-between-identical-usb-to-serial-adapters#50412
using KERNELS attribute (eg 1-4-5 means port 5 of hub on port 4).  eg udev rule
...
$ sudo usermod -a -G dialout charles
