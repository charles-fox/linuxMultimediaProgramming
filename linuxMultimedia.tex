...
\section{OpenAL}
OpenAL was developed by Creative labs (maker of soundcards) to provide a games-focussed interface allowing sounds and listeners to be positioned in space.  Then effects including time delay, volume attenuation, and doppler shifts are implemented to compute the sound at the listener location.

It comes with headphone setups and also standard stereo, 5.1 and 7.1 surround speaker setups, which approximate the locations around a listing chair.

(does it have a head model too to impede the sound across the skull?)
